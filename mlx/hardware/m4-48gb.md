...
- `--prefill-step-size 2048`: best balanced measured prefill setting
...
- Long-prompt memory pressure: try `--prefill-step-size 1024`, then `512`.
...
See [M4 Max 48GB Benchmarks](./m4-48gb-benchmark.md) for measured TTFT, decode speed, cache reuse, and power comparisons.
...
Tested with `mlx-lm 0.31.3`, `mlx 0.31.2`, and macOS 26.5.2. Run `mlx_lm.server --help` after upgrades.
